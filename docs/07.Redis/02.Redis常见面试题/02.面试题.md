---
title: 面试题
date: 2021-07-17 22:45:20
permalink: /pages/70c900/
article: false
---
...
Redis 提供了`多种数据类型`来支持不同的业务场景。Redis 还支持`事务` 、`持久化`、`Lua 脚本`、`多种集群方案`。
...
不过，了解 Redis 和 Memcached 的区别和共同点，有助于我们在做相应的技术选型的时候，能够做到有理有据！
...
![image-20210705160323609](https://pan.layne666.cn/images/2021/07/17/sSBA8fp5RK.png)
...
![img](https://pan.layne666.cn/images/2021/07/17/YxVCpB7itC.png)
...
1. **介绍：** string 数据结构是简单的 key-value 类型。虽然 Redis 是用 C 语言写的，但是 Redis 并没有使用 C 的字符串表示，而是自己构建了一种 **简单动态字符串**（simple dynamic string，`SDS`）。`C 字符串`与 `SDS` 的区别如下：
...
3. **应用场景：** 缓存功能、计数、共享Session、限速、分布式锁等。
...
![img](https://pan.layne666.cn/images/2021/07/17/k6MYdizate.png)
...
![img](https://snailclimb.gitee.io/javaguide/docs/database/Redis/images/redis-all/redis事件处理器.png)
...
![redis过期字典](https://snailclimb.gitee.io/javaguide/docs/database/Redis/images/redis-all/redis过期时间.png)
...
![redis事务](https://snailclimb.gitee.io/javaguide/docs/database/Redis/images/redis-all/redis事务.png)
...
![缓存穿透情况](https://snailclimb.gitee.io/javaguide/docs/database/Redis/images/redis-all/缓存穿透情况.png)
...
![image](https://snailclimb.gitee.io/javaguide/docs/database/Redis/images/redis-all/加入布隆过滤器后的缓存处理流程.png)
